-- Fix duplicate employee inserts when auth.users triggers handle_new_user()
-- and the admin RPC also provisions the employee profile.
-- Note: the return expression is corrected by the next migration:
...
BEGIN
  ...
  SELECT row_to_json(e.*) || jsonb_build_object('private_details', row_to_json(pd.*))::jsonb
  INTO result_record
  FROM public.employees e
